        ...
        <div className="relative z-10 grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div data-motion-card className="rounded-[1.6rem] border border-white/10 bg-white/[0.06] p-5" aria-busy={previewPending}>
            <div className="flex items-center justify-between gap-3">
              <p className="text-caption font-semibold text-white/70">当前实验组合</p>
              <span className="rounded-full bg-white/10 px-3 py-1 text-caption font-semibold text-brand-warm">
                {previewPending ? "正在刷新预览" : "选择即刻预览"}
              </span>
            </div>
            <span className="bz-hero-stat mt-3 border-white/20" style={{ background: "rgba(255,255,255,0.08)" }}>
              <span className="bz-eyebrow-inverse">预期年化</span>
              <span className="text-hero-num tabular-nums text-white">{payload.selectedPlan.expectedReturn}%</span>
            </span>
            <p className="mt-3 text-h3 text-white">{payload.selectedPlan.label}</p>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.7fr)]">
        <section data-fund-item data-motion-reveal className="panel rounded-[2rem] p-5 sm:p-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-h1 text-fg-strong">基金池对比</h2>
          </div>
            ...
            {payload.funds.map((fund) => (
              <article key={fund.id} data-motion-card className="rounded-[1.6rem] border border-slate-200 bg-white p-5 shadow-sm">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-h3 text-fg-strong">{fund.name}</p>
                    <p className="mt-1 text-body-sm text-fg-muted">{fund.type}</p>
                  </div>
                ...
                <p className="mt-4 text-body-sm leading-7 text-fg-muted">{fund.summary}</p>
        ...
        <aside data-fund-item data-motion-reveal className="space-y-6">
          <section className="panel rounded-[2rem] p-5 sm:p-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-h1 text-fg-strong">模拟配置</h2>
            </div>
            <div className="mt-5 grid gap-3">
              {payload.plans.map((item) => (
                ...
                >
                  <p className="text-h3 text-fg-strong">{item.label}</p>
            ...
            <label className="mt-5 block">
              <span className="mb-2 block text-body-sm font-semibold text-fg-strong">模拟金额</span>
              <input
                ...
                className="min-h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-h3 tabular-nums font-semibold text-fg-strong outline-none focus:border-brand"
              />
            </label>
            <label className="mt-4 block">
              <span className="mb-2 block text-body-sm font-semibold text-fg-strong">配置理由</span>
              <textarea
                ...
                className="w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 text-body font-semibold leading-7 text-fg-default outline-none focus:border-brand"
              />
            </label>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              <button
                ...
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-5 text-body-sm font-semibold text-fg-default transition hover:-translate-y-0.5 disabled:opacity-60"
              >
              ...
              <button
                ...
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-full bg-brand px-5 text-body-sm font-semibold text-slate-950 shadow-glow transition hover:-translate-y-0.5 disabled:opacity-60"
              >
          ...
          <section className="panel rounded-[2rem] p-5 sm:p-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-h1 text-fg-strong">配置比例</h2>
            </div>
            <div className="mt-5 space-y-3">
              {allocationRows.map((row) => (
                <div key={row.fundId} className="rounded-[1.35rem] bg-slate-50 p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-body-sm font-semibold text-fg-strong">{row.fund.name}</p>
                      <p className="text-caption text-fg-muted">{row.fund.type}</p>
                    </div>
                    <p className="text-h3 tabular-nums text-brand-ink">{row.weight}%</p>
                  </div>
                  <div className="mt-3 h-2 overflow-hidden rounded-full bg-white">
                    <div className="h-full rounded-full bg-brand" style={{ width: `${row.weight}%` }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {payload.history.length > 0 ? (
            payload.history.slice(0, 3).map((entry) => (
              <article key={entry.id} data-motion-card className="rounded-[1.4rem] border border-slate-200 bg-white p-4">
                <p className="text-h3 text-fg-strong">{entry.planLabel}</p>
                <p className="mt-1 text-body-sm tabular-nums text-brand-ink">
            ...
            <p className="rounded-[1.4rem] border border-dashed border-slate-200 bg-slate-50 p-5 text-body text-fg-muted lg:col-span-3">
...
function Metric({
  ...
  return (
    <div className={cn("rounded-2xl p-3", light ? "bg-slate-50" : "bg-white/10")}>
      <p className={cn("text-caption font-semibold", light ? "text-fg-muted" : "text-white/70")}>{label}</p>
      <p className={cn("mt-1 text-h3 tabular-nums", light ? "text-fg-strong" : "text-white")}>{value}</p>
    </div>
  );
}
