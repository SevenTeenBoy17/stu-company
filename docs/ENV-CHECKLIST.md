...
| `AI_API_KEY` | gpt-agent.cc 或 Anthropic 兼容网关 | ⚠️ | 没有则降级为本地教学兜底 |
| `AI_MODEL` | 模型 ID（默认 claude-sonnet-4-6）| ⚠️ | |
| `AI_BASE_URL_PRIMARY` | 主网关 URL | ⚠️ | 默认 https://gpt-agent.cc/v1 |
...
- 如果用 gpt-agent.cc：注册 → 控制台 → API Keys → 创建 → 填 `AI_API_KEY`
...
| AI 回复总是 "本地教学兜底" | AI_API_KEY 没填或 key 失效 | 检查 gpt-agent.cc 配额 |
