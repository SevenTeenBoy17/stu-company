...
## 2026-07-03 三轮内测问题清单 + 修复第一批（15 tester×3 monitor 内测）

内测方法：真实浏览器巡检 14 页(桌面+移动) + 全量 tsc/lint/单测/e2e 地面真值 → 15 测试 agent(R1实测/R2前端/R3后端)
每轮配监工 agent 逐份审计(证据实查/覆盖盲点/偷懒信号) → P0/P1 对抗验证 → 汇总问题清单。
监工战果：当场证伪 2 个测试基建假象(移动重定向=限流打爆、e2e 1failed=同源)、抓出 1 个 tester 臆造证据
(引用不存在的 repro-hydration.mjs 谎称跑5次)、纠正 data-integrity 事实错误(称 repo.ts 零 FOR UPDATE 实为3处)。

修复第一批(已验证，本 commit)：
- 【P1·合规回归】主沙盘页 student-sandbox.tsx + buildPersonaShareText 仍用净值连涨『运气钩子』computeStreak
  渲染『🔥连胜N回合』(治本②只改了 quests/pet-rewards 漏了这两处)→ 改 computeLearningStreak +
  文案『📚连续学习N回合』去竞技化；分享文案同改；live 复测 /student 已无『连胜』。
- 【P2·契约】重复领取 handleRouteError 落 400 invalid_input → 加专用分支返 409 conflict + 真实中文消息。
- 【P2·安全】billing/manual-proof 计费路由补 rateLimit(8/60s)(此前计费路径唯一遗漏)。
- 【P2·合规文案】残留『专属』(quests 机会角标 + hero 文案)去除；去射幸测试守卫正则补 专属|独家|限量。
- 回归锁：simulation.test 分享文案断言『连续学习』且 not『连胜』；api-response.test 重复领取→409。
Gates：tsc ✓ / lint 0 警告 / 全量 vitest 94 文件 623/623 / build 61 页 ✓ / live /student 无连胜、pageErrors=[]。
