      ...
      <section data-credit-reveal data-motion-reveal className="overflow-hidden rounded-[2rem] bg-bg-inverse text-white shadow-soft">
        <div className="relative grid gap-0 xl:grid-cols-[minmax(0,1.15fr)_minmax(340px,0.85fr)]">
          ...
          <div className="relative z-10 px-6 py-7 md:px-8 md:py-9">
            <p className="text-sm font-bold uppercase tracking-[0.24em] text-brand-warm">Credit Lab</p>
          ...
          <aside className="relative z-10 border-t border-white/10 bg-white/[0.04] px-6 py-7 md:px-8 xl:border-l xl:border-t-0">
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <h2 className="mt-3 text-h1 font-semibold">{payload.coach.title}</h2>
              </div>
              ...
              {payload.coach.nextSteps.map((step, index) => (
                <div key={step} className="rounded-[1.35rem] border border-white/10 bg-white/[0.07] p-4">
                  <div className="flex gap-3">
                    <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-brand text-sm font-black">
                      {index + 1}
                    </span>
                    <p className="text-sm font-semibold leading-6 text-white/72">{step}</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div data-credit-reveal data-motion-reveal className="panel rounded-[2rem] p-5 md:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-h1 font-semibold text-slate-950">信用场景卡</h2>
              </div>
              <p className="mt-2 max-w-2xl text-body leading-7 text-slate-600">
                先模拟，再执行。执行借款或还款会写入沙盘历史，但不会凭空提高净值。
              </p>
            </div>
            <span className={cn("rounded-full border px-4 py-2 text-sm font-black", statusClass[selected.status])}>
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
            ...
            <div className="rounded-[1.7rem] border border-slate-200 bg-slate-50 p-5">
              <label className="text-sm font-black text-slate-950" htmlFor="credit-amount">
                操作金额
              </label>
              <input
                id="credit-amount"
                ...
                className="mt-3 h-12 w-full rounded-2xl border border-slate-200 bg-white px-4 text-base font-bold text-slate-950 outline-none transition focus:border-brand"
                inputMode="numeric"
              />
              <div className="mt-4 grid gap-2">
                <button
                  ...
                  className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-slate-200 bg-white px-4 text-sm font-black text-slate-950 transition hover:border-brand disabled:opacity-60"
                >
                ...
                <button
                  ...
                  className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full bg-slate-950 px-4 text-sm font-black text-white transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-40"
                >
        ...
        <aside data-credit-reveal data-motion-reveal className="space-y-6">
          <div className="panel rounded-[2rem] p-5 md:p-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-h1 font-semibold text-slate-950">还款雷达</h2>
            </div>
          ...
          {(message || result) && (
            ...
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h2 className="text-h2 font-black text-slate-950">操作反馈</h2>
